...
const (
	...
	InfluxDB2Org    = "my-org"
	InfluxDB2Token  = "i3EE29aNIQqWPFHzUUTzvp83gZiro1ZQuAXFlESch2dG_8YUUlDWqtKSxwdMu7xXtlBDo9e1UHZ976GCmJ4PFg=="
)
...
type WriterV2B struct {
	influx   *clientb.InfluxDBClient
	writeApi clientb.WriteApi
}

func main() {
	writerType := flag.String("type", "CLIENT_GO_V2B", "Type of writer (default 'CLIENT_GO_V2B'; CLIENT_GO_V1, CLIENT_GO_V2, CLIENT_GO_V2B)")
	...
	if *writerType == "CLIENT_GO_V2B" {
		...
		writer = &WriterV2B{influx: influx, writeApi: influx.WriteAPI(InfluxDB2Org, InfluxDB2Bucket)}
	} else {
